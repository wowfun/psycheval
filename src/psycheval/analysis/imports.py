...
from psycheval.analysis.constants import (
...
from psycheval.config import PEVAL_CONFIG_FILENAME
...
def ensure_import_workspace_root(value: str | Path) -> Path:
    ...
    if not config_path.is_file():
        raise ValueError(
            f"{root} is not an initialized peval workspace; run `peval init -r {root}`"
        )
    return root

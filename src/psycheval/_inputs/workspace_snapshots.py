...
import psycheval.config as path_config
from psycheval._state.annotations import (
...
def infer_workspace_root_from_trial_cell_paths(raw_paths: list[str]) -> Path | None:
    ...
    if len(roots) > 1:
        ...
        raise ValueError(
            "path inputs belong to different inferred peval workspace roots: "
...
def infer_workspace_root_from_trial_cell_path(raw_path: str) -> Path | None:
    ...
    if path is None:
        return None
    for candidate in [path, *path.parents]:
        config_path = candidate / "peval.toml"
...
def workspace_snapshot_source_for_artifact_path(
    ...
    if not (cell_path / SOURCE_STATE_DIR / SOURCE_STATE_FILENAME).is_file():
        return None
    from psycheval.state import open_workspace_state_readonly
...
def load_workspace_snapshot_sessions_from_rows(
    ...
    *,
    ...
    if not workspace_root:
        raise ValueError(
            f"{raw_input} belongs to a peval workspace, but no workspace root is configured"
        )
    from psycheval.state import open_workspace_state_readonly

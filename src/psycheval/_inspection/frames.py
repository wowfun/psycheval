...
from psycheval._inspection.utils import (
    compact_text,
    final_metric,
    idle_duration,
    int_or_zero,
...
def source_row(
    ...
    return {
        ...
        "display_alias": meta.get("display_alias"),
        "source_tags": meta.get("source_tags") or [],
        "display_tags": meta.get("display_tags") or [],
        ...
        "harbor_step": meta.get("harbor_step")
        if isinstance(meta.get("harbor_step"), dict)
        else {},
        "harbor_trial_evaluation": meta.get("harbor_trial_evaluation")
        if isinstance(meta.get("harbor_trial_evaluation"), dict)
        else {},
        "task_metadata": meta.get("task_metadata") or {},
        "trajectory_available": meta.get("trajectory_available", True),
        "diagnostic": meta.get("diagnostic"),
        "failure": (
            meta.get("evaluation", {}).get("exception")

...
from psycheval._inputs.workspace_snapshots import optional_text as optional_text
from psycheval._inputs.workspace_snapshots import (
    ...
    *,
    ...
    for index, path in enumerate(paths, start=1):
        ...
        harbor_bundle = (
            ...
            and isinstance(config, ToolConfig)
            else None
        )
        if harbor_bundle is not None:
            ...
            documents = project_harbor_trial_bundle(harbor_bundle)
            unreadable = [
                document for document in documents if document.trajectory is None
            ]
            if unreadable:
                first = unreadable[0]
                raise ValueError(
                    "raw Harbor Trial view requires a trajectory for every source: "
                    f"{first.last_error or first.source_ref}"
                )
            for document in documents:
                assert document.trajectory is not None and document.meta is not None
                sessions.append(
                    LoadedSession(
                        records=None,
                        input_label=str(document.source.get("label") or path),
                        adapter_id="harbor",
                        input_path=str(harbor_bundle.trial_dir),
                        session_hint=optional_text(
                            document.trajectory.get("session_id")
                        ),
...
def adapter_for_input_path(
    ...
    if inferred is None and adapter_assignments.require_inferred_default:
        options = ", ".join(sorted(available)) or "<none>"
        raise ValueError(

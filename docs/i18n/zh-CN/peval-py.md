...
Alias 会提升 source list、Trajectory Overview 和 selected Trial 的可读性，并显示在
Leaderboard 独立的 Session Alias 列；canonical Session 列保持不变。它不会改变
...
支持 alias，每个已保存 source 行都有 alias 编辑器。Source 行还会显示可排序的
Last Turn End，来源是已保存 Trial 的 `trajectory_meta.finished_at_ms`。Alias 与
source identity 分开存储，并且可以清空。
...
显示 Analysis section。在 `serve` 中，active report 组合会把当前 workspace 里的
analysis 和 cell notes 叠加到已保存 source snapshot 上；因此 reload 或显式
Refresh 即使遇到原始 source DB/file 无法成功刷新，也能显示 `analysis.json`、
`analysis.md` 或 `notes.md` 的变化。持久化 trajectory snapshot 仍然是最后一次成功
source conversion 的结果。

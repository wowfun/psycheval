...
- Python `>=3.12` and the exact stable dependency `harbor==0.20.0` are required.
...
The supported upstream contract is the stable Harbor `0.20.0` release. Local
...
- A deterministic external-harness Trial produces ATIF, checks, reward, and
  artifacts through Harbor 0.20.0.

---
name: 001. peval-py Testing
---

# 001. peval-py Testing
...
- serve HTTP `POST /api/sources` accepts shell-quoted multi-path strings for
  path and DB payloads, rejects malformed quoted input clearly, requires `auto`
  or omitted adapter selection to infer one adapter from source path tokens when
  conversion needs an adapter, and persists no new source/trial/log rows when a
...
- Trajectory Overview renders one session row per currently filtered and sorted
  ...
  internal scroll positions. Coverage includes the complete serve detail
  selection path that replaces the comparison shell after loading a different
  Trial. Leaderboard and Trajectory Overview vertical scroll progress stays
  synchronized in both directions while keeping Leaderboard horizontal scroll
  independent.
...
No Rust workspace validation is part of the standalone `psycheval` default
validation path.

## Related Topics

- [001 peval-py](spec.md)

...
Source Manager Path, DB, and input-table imports follow the same hard-fail rule
when the adapter control is `auto` or omitted: peval-py must infer one adapter
from the source path tokens or fail clearly and ask the user to choose an
adapter. Input-table row adapters still override the form default for that row;
rows without an adapter must infer from their row `path` or `db`. ATIF JSON,
peval-py report JSON, complete Trial cells, and recursive `runs/` imports remain
adapter-free snapshot/artifact inputs when they do not need message or DB
conversion.
...
For direct CLI `-p` and `-d` inputs without a per-input selector and without a
bare CLI `-a ADAPTER`, `peval-py` may infer the adapter from the path before
falling back to the config/default adapter. Inference is generic over available
adapter ids: an adapter id must appear as a complete path component or filename
token, not as an arbitrary substring; hidden directory names such as `.hermes`
and `.psychevo` are tokenized the same way. If multiple adapter ids match one
path, inference fails clearly and asks for `-a`. ATIF and peval-py report JSON

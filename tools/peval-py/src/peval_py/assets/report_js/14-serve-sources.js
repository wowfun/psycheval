...
function sourceColumns() {
  return [
    { key: "__source_select", width: "46px", sourceSelect: true, label: t("select_rows", "Select rows"), value: source => source?.source_key || "", html: renderSourceSelection },
...
function renderSourceSelectionHeader(rows) {
  ...
  return `<th class="select-col"><label class="select-box"><input type="checkbox" data-source-select-visible ${checked ? "checked" : ""} ${partial ? "data-partial=\"true\"" : ""} aria-label="${esc(t("select_visible_sources", "Select visible sources"))}"><span></span></label></th>`;
}
...
function sourceSelectionKeys(rows = sourceRows()) {
  const visible = new Set(sourceVisibleKeys(rows));
  return Array.from(state.sourceSelection).filter(key => visible.has(key));
}
function sourceSelectedRows(rows = sourceRows()) {
  const selected = new Set(sourceSelectionKeys(rows));
  return rows.filter(source => source?.source_key && selected.has(source.source_key));
}
...
function pruneSourceSelection() {
  const known = new Set(listValue(state.serveSources).map(source => String(source?.source_key || "").trim()).filter(Boolean));
  Array.from(state.sourceSelection).forEach(key => {
    if (!known.has(key)) state.sourceSelection.delete(key);
  });
}

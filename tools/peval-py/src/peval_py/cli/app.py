...
APP_HELP = (
    "Lightweight standalone Python peval. Current scenarios init, export, "
    "import, view, or serve retained agent trajectories and analysis artifacts."
)
INSPECT_EPILOG = """\
...
"""
OUTPUT_DEFAULT_MARKER = "\0peval-py-default-output"
...
import_app = make_app(help="Import analysis files into peval-py workspace artifacts.")
...
def execute(args: CliArgs) -> None:
    from peval_py.cli.main import run_cli_args
...
@app.command(
    "init",
    help="Create or repair the minimal peval-py serve state.",
    short_help="Initialize peval-py serve state",
)
def init_command(
    root: Annotated[
        str | None,
...
@app.command(
    "serve",
    help=(
        "Start the local peval-py saved workspace UI. Source flags persist and "

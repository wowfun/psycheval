...
class PevalPyCliInputTableExportTests(unittest.TestCase):
    ...
    def test_cli_view_export_alias_smoke_and_legacy_commands_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("source path: JSONL", result.stdout)
            self.assertIn("trajectory artifact", result.stdout)
            self.assertIn("Trial cell", result.stdout)
            ...
            self.assertIn("bound source content", result.stdout)
            self.assertIn("inspect preview text", result.stdout)
            self.assertIn("--steps", result.stdout)
            self.assertIn("show selected step_id evidence only", result.stdout)
            self.assertIn("--tool-call", result.stdout)
            self.assertNotIn("--preview-chars", result.stdout)
            self.assertNotRegex(result.stdout, r"--step\b")

...
class PevalPyCliInputTableExportTests(unittest.TestCase):
    ...
    def test_cli_view_export_alias_smoke_and_legacy_commands_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("source path: jsonl", help_text)
            self.assertIn("trajectory artifact", help_text)
            self.assertIn("trial cell", help_text)
            self.assertIn("-i", help_text)
            self.assertIn("--input-table", help_text)
            self.assertIn("-n", help_text)
            self.assertIn("--note", help_text)
            self.assertIn("--head", help_text)
            self.assertIn("--tail", help_text)
            self.assertIn("--top", help_text)
            self.assertIn("bound source content", help_text)
            self.assertIn("inspect preview text", help_text)
            self.assertIn("--steps", help_text)
            self.assertIn("show selected step_id evidence only", help_text)
            self.assertIn("--tool-call", help_text)
            self.assertNotIn("--preview-chars", help_text)
            self.assertNotRegex(help_text, r"--step\b")
            self.assertNotIn("--on", help_text)
            self.assertIn("raw mode only", help_text)
            self.assertIn("--agent-name", help_text)
            self.assertNotIn("--trajectory-id", help_text)

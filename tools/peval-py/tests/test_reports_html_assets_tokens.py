...
class PevalPyReportHtmlAssetTokenTests(unittest.TestCase):
    ...
    def test_html_inlines_template_css_and_js_package_assets(self) -> None:
        ...
        self.assertIn('if ($("leaderboard-summary")) renderLeaderboardSummary(rows);', js)
        ...
        self.assertIn("function leaderboardSummaryGroups(rows = leaderboardRows()", js)
        self.assertIn("function measuredModelDurationForRow(row)", js)
        self.assertIn("function leaderboardSummaryStatistics()", js)
        self.assertIn("function renderLeaderboardSummaryTableDisclosure(groups)", js)
        self.assertIn("function renderLeaderboardSummaryCharts(groups)", js)
        self.assertIn("function renderLeaderboardSummaryChart(definition, groups, statistic)", js)
        self.assertIn("function setLeaderboardSummaryStatistic(value)", js)
        ...
        self.assertIn("const toggleDisabled = allMode", js)
        self.assertNotIn("targetReadableCount", js)
        self.assertNotIn("readableServeSources(nextMode).length", js)

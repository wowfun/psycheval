...
const fetchStub = async (path, options = {}) => {
  ...
  if (value === "/api/acp/agents") {
    ...
    payload = { prompts: [{ id: "failure-diagnosis", title: "Failure diagnosis", content: "# Failure diagnosis\n\nTrace the first mistake." }] };
  } else if (value.startsWith("/api/acp/sessions?")) {
    payload = { sessions: [
      { agent_id: "opencode", session_id: "session-1", title: "Investigation", active_prompt: false, closed: false, loaded: true, revision: 0 },
      { agent_id: "opencode", session_id: "session-2", title: "Needs resume", active_prompt: false, closed: false, loaded: false, revision: 0 },
    ] };
  } else if (value === "/api/acp/sessions" && options.method === "POST") {
    return jsonResponse({ error: "resume failed" }, 502);
  } else if (value.startsWith("/api/acp/events?")) {
    eventRequest += 1;
    if (eventRequest === 2) {
    ...
    payload = {
      revision: 3,
      reset: false,
      session: { agent_id: "opencode", session_id: "session-1", title: "Investigation", active_prompt: false, closed: false, loaded: true, revision: 3 },
      events: [
        { revision: 1, type: "message", text: "**Failure cluster** found." },
        { revision: 2, type: "tool", title: "Read trajectory", status: "completed", raw_input: { source: "source-7" } },
        { revision: 3, type: "permission", request_id: "91", options: [{ optionId: "allow_once", name: "Allow once" }] },
      ],
    };
  } else if (value === "/api/acp/permission") {
    payload = {};
  }
...
test("ACP drawer preserves explicit context, renders protocol events, and restores focus", async () => {
  ...
  await tick();
  assert.deepEqual(calls.find(call => call.path === "/api/acp/permission").body, {
    agent_id: "opencode", session_id: "session-1", request_id: "91", option_id: "allow_once", cancelled: false,
  });
  ...
  resolveRecoveredPoll(jsonResponse({
    revision: 0,
    reset: true,
    session: { agent_id: "opencode", session_id: "session-1", title: "Investigation", active_prompt: false, closed: false, loaded: true, revision: 0 },
    events: [],
  }));
  await tick();
  ...
  resolvePermissionPoll(jsonResponse({
    revision: 4,
    reset: true,
    session: { agent_id: "opencode", session_id: "session-1", title: "Investigation", active_prompt: false, closed: false, loaded: true, revision: 4 },
    events: [
      { revision: 3, type: "permission", request_id: "91", options: [{ optionId: "allow_once", name: "Allow once" }] },
      { revision: 4, type: "permission_result", request_id: "91", option_id: "allow_once", cancelled: false },
    ],
  }));
  await tick();
  ...
  await tick();
  assert.ok(
    calls.some(call => call.path.startsWith("/api/acp/events?") && call.path.includes("session_id=session-2")),

...
class PevalCliInputInspectTests(unittest.TestCase):
    def test_inspect_rejects_raw_report_notes_and_source_aliases(self) -> None:
        from psycheval.cli import main

        for option in ("--note", "--source-alias"):
            with self.subTest(option=option):
                ...
                self.assertIn(
                    f"raw-only option(s) require -m raw: {option}",
                    stderr.getvalue(),
                )
    ...
    def test_top_token_ranking_does_not_add_cached_prompt_subset_twice(self) -> None:
        ...
        from psycheval._inspection.payload import source_payload
    ...
    def test_cli_view_inspect_is_default_fixed_digest(self) -> None:
        ...
        self.assertEqual(payload["inspect_schema_version"], 3)

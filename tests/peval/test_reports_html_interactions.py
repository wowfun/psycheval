...
class PevalReportHtmlInteractionTests(unittest.TestCase):
...
const nodes = {
  ...
  "peval-i18n": { textContent: JSON.stringify({
  ...
  "peval-render-options": { textContent: JSON.stringify({ mode: "serve", loading: true, sources: [] }) }
};
const context = {
  document: {
...
const nodes = {{
  ...
  "peval-token-estimates": {{ textContent: "{{}}" }},
  "peval-render-options": {{ textContent: JSON.stringify({{
    mode: "serve",
...
nodes["peval-data"].textContent = "{}";
nodes["peval-i18n"].textContent = "{}";
nodes["peval-token-estimates"].textContent = "{}";
nodes["peval-render-options"].textContent = JSON.stringify({ mode: "serve", sources: [] });
const context = {
  nodes,
  document: {
    body: { classList: { add() {}, remove() {}, toggle() {} } },
...
const context = {
  document: {
    ...
    addEventListener() {},
    getElementById(id) {
      if (id === "peval-i18n") return { textContent: "{}" };
      if (id === "peval-token-estimates") return { textContent: "{}" };
      if (id === "peval-render-options") return { textContent: JSON.stringify({ mode: "serve", sources }) };
      return null;
    },
    querySelector(selector) {
...
""".replace("__ASSET__", json.dumps(asset))
        ...
        self.assertEqual(
            ...
            ["Delete selected sources from peval state?"],
        )
        self.assertEqual(

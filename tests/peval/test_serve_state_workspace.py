...
from psycheval.cli import main as cli_main
...
class PevalServeStateWorkspaceTests(unittest.TestCase):
    def test_workspace_discovery_and_missing_workspace_diagnostics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                self.assertTrue((created / "peval.toml").is_file())
                self.assertTrue((created / "logs").is_dir())
                self.assertFalse((created / "state.db").exists())
                self.assertFalse((created / "peval-py.toml").exists())
            finally:
                store.close()
            ...
            try:
                ...
                with self.assertRaisesRegex(
                    ValueError, "peval workspace is not initialized"
                ):
                    resolve_workspace_root()
            finally:
                os.chdir(old_cwd)
    ...
    def test_legacy_workspace_config_filename_is_not_discovered(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
    ...
    def test_workspace_state_refresh_and_archive_lifecycle(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertEqual(source_payload["kind"], "trial-artifact")
                self.assertFalse(source_payload["refreshable"])
                self.assertTrue(source_payload["snapshot"])
                self.assertTrue(
                ...
                self.assertFalse(reopened_source["refreshable"])
                store.refresh_sources(keys, config)
                self.assertEqual(
    ...
    def test_legacy_flat_source_state_is_read_and_rewritten_as_minimal_overlay(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertNotIn("schema_version", rewritten)
    ...
    def test_report_json_upload_is_non_refreshable_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertEqual(analysis_json["commands"], ["peval view tr -f json"])
    ...
    def test_artifact_source_overlay_file_is_removed_when_last_field_is_cleared(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store = open_workspace_state(str(root))
            try:
                source_key = store.source_payload()[0]["source_key"]
